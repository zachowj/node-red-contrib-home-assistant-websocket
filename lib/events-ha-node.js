...
const { INTEGRATION_LOADED, INTEGRATION_UNLOADED } = require('./const');
...
class EventsHaNode extends EventsNode {
    ...
    async onHaIntegration(type) {
        ...
        } else if (type === INTEGRATION_UNLOADED) {
            this.isEnabled = true;
            this.removeSubscription();
    ...
    async handleTriggerMessage(data = {}) {
        ...
        try {
            ...
            entity = await this.nodeConfig.server.homeAssistant.getStates(
                entityId
            );
